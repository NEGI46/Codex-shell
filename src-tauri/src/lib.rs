...
fn canonical_project_root(raw_path: &str) -> Result<PathBuf, String> {
    ...
    let root = path.canonicalize().map_err(|_| "プロジェクトフォルダが見つかりません".to_string())?;
    if !root.is_dir() {
...
#[tauri::command]
fn register_project(state: State<ShellState>, name: String, path: String) -> Result<Project, String> {
    ...
    state.projects.lock().map_err(|_| "状態のロックに失敗しました")?.push(project.clone());
    Ok(project)
}

#[tauri::command]
fn list_projects(state: State<ShellState>) -> Result<Vec<Project>, String> {
    Ok(state.projects.lock().map_err(|_| "状態のロックに失敗しました")?.clone())
}

#[tauri::command]
fn create_session(state: State<ShellState>, project_id: String) -> Result<NativeSession, String> {
    let projects = state.projects.lock().map_err(|_| "状態のロックに失敗しました")?;
    ...
    state.sessions.lock().map_err(|_| "状態のロックに失敗しました")?.insert(session.id.clone(), session.clone());
    Ok(session)
}

#[tauri::command]
fn start_app_server(app: AppHandle, state: State<ShellState>, project_path: String) -> Result<(), String> {
    let root = canonical_project_root(&project_path)?;
    let mut runtime = state.runtime.lock().map_err(|_| "状態のロックに失敗しました")?;
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![register_project, list_projects, create_session, start_app_server, codex_available])
